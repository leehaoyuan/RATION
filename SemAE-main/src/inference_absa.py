...
if __name__ == '__main__':
    ...
    data_arg_group.add_argument('--summary_data',
                                ...
                                type=str,
                                default='../data/space/json/space_summ.json')
    data_arg_group.add_argument('--pair_data',
                                help='summarization benchmark data',
                                type=str,
                                default='../data/space/json/space_summ.json')
    ...
    with open(args.pair_data,'r') as file:
        pair=json.load(file)
    ...
    with torch.no_grad():
        for entity_id, entity_loader in tqdm(summ_dls.items()):
            ...
            for batch in entity_loader:
                ...
                #print(src.size())
                for full_id in ids:
                    entity_id, review_id = full_id.split('\n\n')
            ...
            for i in range(len(texts)):
                if texts[i] in pair[entity_id].keys() and len(pair[entity_id][texts[i]])>0:
                    noun_adj_mask.append(1)
                    count+=1
                else:
                    noun_adj_mask.append(0)
            print(count)
            noun_adj_mask=np.array(noun_adj_mask)
            ...
            dist = dist[noun_adj_mask==1]
            texts= [texts[j] for j in range(len(texts)) if noun_adj_mask[j]==1]
    ...
    for entity_id, ranked_sentences in tqdm(ranked_entity_sentences.items()):
        ...
        fout.close()
    #media={'text':all_texts_list,'dist':all_dist,'id':all_ids,'entity':all_entity}
    #with open('media_clause.pickle','wb') as file:
        #pickle.dump(media,file)
    # evaluate summaries
