...
if __name__=="__main__":
    ...
    for i in os.listdir(hp.summary_path):
        media_key=None
        if i.startswith('test_'):
            ...
            media_key=i[4:]
        result[media_key]={}
        with open(os.path.join(hp.summary_path,i),'r') as file:
            sents1=file.read().strip().split('\t')
        sents1=[j.strip() for j in sents1 if len(j)>1]
